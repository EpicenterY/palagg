...
Manifold-3D WASM CSG는 메인 스레드에서 실행되므로, 슬라이더 드래그 시 프레임 블로킹을 최소화하는 최적화가 적용되어 있다.

| 최적화 | 위치 | 설명 |
|--------|------|------|
| 다운로드 모델 디바운스 | `main.ts` `TMF_DEBOUNCE_MS` | 슬라이더 드래그 중 다운로드용 CSG 빌드 지연 (300ms) — 프리뷰 CSG만 실행 |
| 리빌드 쓰로틀 | `main.ts` `MODEL_REBUILD_INTERVAL` | CSG 완료 후 최소 간격 (16ms) 대기 |
| Fill Pass 조건부 실행 | `outline/index.ts` `fillEnabled` | Box/Grid에서 fill 서브패스 비활성화 (씬 렌더 3→1회) |
| 빠른 노멀 계산 | `main.ts` `reloadModel()` | Box/Grid는 `computeVertexNormals()`, Tag만 `computeCreaseNormals()` |
| 경량 지오메트리 | `manifold.ts` `N_SEGMENTS=10` | 라운드 코너 아크 세그먼트 수 최적화 |
| 애니메이션 흡수 | `animate.ts` `easeOutCubic` | 연속 입력을 보간으로 흡수, 즉시 출발→감속 (0.55s) |

#### Manifold-3D WASM 주의사항

- **WASM 객체 캐싱 금지**: `Manifold`, `CrossSection` 등 WASM 바인딩 객체는 CSG 호출 간 재사용 불가. 캐싱 시 `"Expected null or instance of Manifold"` 에러 발생.
- 순수 JS 데이터(꼭짓점 배열 `CLIP_R_VERTICES` 등)만 모듈 레벨 상수로 저장 가능.
